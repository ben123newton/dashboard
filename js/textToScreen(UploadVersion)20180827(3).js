...
function createProgressTbl(){
	...
	for(var i = 0; i<2;i++){
		var headerCell = document.createElement('th');
		var headerCellText = (i==0)? document.createTextNode('Month'):document.createTextNode('Progress');
		headerCell.appendChild(headerCellText);
		headerRow.appendChild(headerCell)
	}
	progressHeader.appendChild(headerRow);
	progressTable.appendChild(progressHeader);
...
function createConsiderateConstructorsTable(location){
	...
	for(var i=0;i<2;i++){
		...
		var colTitle = (i==0)? document.createTextNode('Date'):document.createTextNode('Score');
...
function tblAccidentTrade(location){
	...
	for(var j=0;j<rowNum;j++){
		...
		for(var k=0; k<2;k++){
			...
			switch(k){
				case 0:
					bodyRowCellText = document.createTextNode(tradeData[j].split(/(?=[A-Z])/).join(" "));
					bodyRowCell.appendChild(bodyRowCellText);
					break;
				case 1:
